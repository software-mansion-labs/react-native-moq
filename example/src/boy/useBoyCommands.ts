...
/**
 * Owns the held-button set and auto-repeat loop, encoding presses as JSON on the
 * `command` data track. Each change sends the held set sorted by name; while any
 * button stays held past 300 ms it is re-sent every 10 ms.
 */
export function useBoyCommands(dataTrack: DataTrack, enabled: boolean) {
  ...
  // Drop held state when disabled so the repeat loop never outlives publishing.
  useEffect(() => {
    if (enabled) return;
    held.current.clear();
