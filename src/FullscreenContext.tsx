 ...
 * custom chrome you pass via `<VideoView controls={...} />` can read it too
...
export interface FullscreenControlsAPI {
  ...
  exit(): void;
  /** The player driving this VideoView. */
  player: Player;
}
 ...
 * Throws if used outside a VideoView fullscreen modal — controls only make
...
export function useFullscreenControls(): FullscreenControlsAPI {
  ...
  if (!ctx) {
    throw new Error(
      'useFullscreenControls() must be used inside a VideoView ' +
